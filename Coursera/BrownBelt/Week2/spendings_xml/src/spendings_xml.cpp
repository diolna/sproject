...
vector<Spending> LoadFromXml(istream& input) {
  // Реализуйте эту функцию с помощью библиотеки xml.h
	...
	Document doc1 = Load(input);
	Node root1 = doc1.GetRoot();


	vector<Node> children = root1.Children();
	for(auto& x : children){
		Spending s;
		s.amount = x.AttributeValue<int>("amount");
		s.category =x.AttributeValue<string>("category");
		result.push_back(s);
	}
	return result;
