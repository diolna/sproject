...
public:
  ...
  void Read(int user_id, int page_count) {
    ...
    user_page_counts_[user_id] = page_count;  // O(Q);
    int& position = user_positions_[user_id];
   // cout << "position" << position<< "max user count " << MAX_USER_COUNT_+1 << endl;
  ...
  double Cheer(int user_id) const {
    ...
    cout << "user_coutn =" << user_count << "pos = " << position << endl;
...
private:
  ...
  int GetUserCount() const {
	  LOG_DURATION("getusercount");
    return sorted_users_.size();
  }
  void AddUser(int user_id) {
	  LOG_DURATION("adduser");
    sorted_users_.push_back(user_id);
    user_positions_[user_id] = sorted_users_.size() - 1;
  }
  void SwapUsers(int lhs_position, int rhs_position) {
	  LOG_DURATION("swapusers");
