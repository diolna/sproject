...
template <class It> shared_ptr<Node> ParseComparison(It& current, It end) {
  ...
  if (column.value == "date") {
	...
	//cout << "call DateComparisonNode" << endl;
...
shared_ptr<Node> ParseExpression(It& current, It end, unsigned precedence) {
  ...
  if (current->type == TokenType::PAREN_LEFT) {  // проверяем наличие скобки. если есть, то надо пропустить ее
    ++current; // consume '('
    left = ParseExpression(current, end, 0u);							// вызываем функцию опять.
  ...
  const map<LogicalOperation, unsigned int> precedences = {
